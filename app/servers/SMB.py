...
def ParseShare(data):
    ...
    if a:
        output_data.append(f"[SMB] Requested Share: {a.group(0).decode('UTF-16LE')}")
    ...
    if data[14:16] == "\x03\x80":
        ...
        if ChainedCmdOffset == "\x75":
            ...
            if PassLen > 2:
                ...
                output_data.append(f"[SMB] Clear Text Credentials: {User} : {Password}")
    ...
    def handle(self):
        ...
        try:
            ...
            while True:
                ...
                if data[8:10] == "\x72\x00":  # Negotiate Protocol Response
                ...
                if data[8:10] == "\x73\x00":  # Session Setup AndX Request
                    ...
                    if data[8:10] == "\x73\x00":  # STATUS_SUCCESS
                        if Is_Anonymous(data):
                            Header = SMBHeader(cmd="\x73", flag1="\x98", flag2="\x01\xc8", errorcode="\x72\x00\x00\xc0", pid=pidcalc(data), tid="\x00\x00", uid=uidcalc(data), mid=midcalc(data))
                            Body = SMBSessEmpty()
                            Packet = str(Header) + str(Body)
                            Buffer = struct.pack(">i", len(''.join(Packet))) + Packet
                            self.request.send(Buffer)
            ...
            output_data.append(f"[SMB] Timeout with {self.client_address[0]}")
    ...
    def handle(self):
        ...
        try:
            ...
            if data[0] == "\x81":  # session request 139
                Buffer = "\x82\x00\x00\x00"
                self.request.send(Buffer)
                output_data.append(f"[SMB1LM] Session request 139 received from {self.client_address[0]}")
                data = self.request.recv(1024)
            ...
            if data[8:10] == "\x72\x00":  # Negotiate proto answer
                ...
                Buffer = struct.pack(">i", len(''.join(Packet))) + Packet
                self.request.send(Buffer)
                output_data.append(f"[SMB1LM] Negotiate protocol answer sent to {self.client_address[0]}")
                data = self.request.recv(1024)
            ...
            if data[8:10] == "\x73\x00":  # Session Setup AndX Request
                if Is_LMNT_Anonymous(data):
                    ...
                    Buffer = struct.pack(">i", len(''.join(Packet))) + Packet
                    self.request.send(Buffer)
                    output_data.append(f"[SMB1LM] Anonymous session setup request from {self.client_address[0]}")
                else:
                    ...
                    Buffer = struct.pack(">i", len(''.join(Packet))) + Packet
                    self.request.send(Buffer)
                    output_data.append(f"[SMB1LM] NTLM hash parsed and session setup request sent to {self.client_address[0]}")
                    data = self.request.recv(1024)

        except Exception as e:
            output_data.append(f"[SMB1LM] Error: {str(e)}")
            self.request.close()
